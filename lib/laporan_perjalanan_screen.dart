...
class _LaporanDriverScreenState extends State<LaporanDriverScreen> {
  ...
  Future<void> _fetchTripDetailsAndProceed({bool forceShowForm = false}) async {
    ...
    try {
      ...
      if (trip == null) {
        setState(() { _isLoading = false; _error = "Gagal memuat data trip."; });
        return;
      }
  ...
  bool _hasSubmittedDocsForPage(Trip trip, int page) {
      switch (page) {
        case 0: return !_isStringNullOrEmpty(trip.startKmPhotoPath);
        case 3: return !_isStringNullOrEmpty(trip.muatPhotoPath);
        case 4: return !_isStringNullOrEmpty(trip.deliveryOrderPath);
        case 7: return !_isStringNullOrEmpty(trip.endKmPhotoPath);
  ...
  Future<void> _handleNextPage() async {
    ...
    try {
      ...
      if (needsVerification) {
        await _navigateToVerification(submittedTrip, isRevisionResubmission: wasRevision);
      } else {
    ...
    if (!isRevision) {
      if (!(_formKey.currentState?.validate() ?? false) || _kmAwalImageFile == null) {
        if (_kmAwalImageFile == null) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Foto KM Awal tidak boleh kosong'), backgroundColor: Colors.red));
        }
        return null;
      }
    } else {
      if (widget.trip.startKmPhotoStatus.isRejected && _kmAwalImageFile == null) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Silakan unggah ulang foto KM Awal yang ditolak.'), backgroundColor: Colors.orange));
        return null;
      }
    }
      ...
      children: [
        ...
        if (!suratJalanStatus.isApproved)
          _MultiPhotoSection(
            ...
            onImagesChanged: (files) => setState(() => _suratJalanImages = files), 
    ...
    return Column(
      ...
      children: [
        ...
        if (!doStatus.isApproved)
          _PhotoSection(title: 'Foto Delivery Order (DO)', icon: Icons.receipt_long_outlined, onImageChanged: (file) => setState(() => _deliveryOrderImage = file), rejectionReason: doStatus.rejectionReason, isApproved: doStatus.isApproved),
        if (doStatus.isApproved)
          _ApprovedDocumentPlaceholder(title: 'Foto Delivery Order (DO)'),
        const SizedBox(height: 24),
        if (!segelStatus.isApproved)
          _PhotoSection(title: 'Foto Segel', icon: Icons.shield_outlined, onImageChanged: (file) => setState(() => _segelImage = file), rejectionReason: segelStatus.rejectionReason, isApproved: segelStatus.isApproved),
        if (segelStatus.isApproved)
          _ApprovedDocumentPlaceholder(title: 'Foto Segel'),
        const SizedBox(height: 24),
        if (!timbanganStatus.isApproved)
          _PhotoSection(title: 'Foto Timbangan Kendaraan', icon: Icons.scale_outlined, onImageChanged: (file) => setState(() => _timbanganImage = file), rejectionReason: timbanganStatus.rejectionReason, isApproved: timbanganStatus.isApproved),
        if (timbanganStatus.isApproved)
    ...
    return Form(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 24),
          TextFormField(enabled: isFormEnabled, controller: _endKmController, decoration: InputDecoration(labelText: 'KM Akhir Kendaraan', filled: true, fillColor: Colors.grey[100], border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none), prefixIcon: const Icon(Icons.speed_outlined)), keyboardType: TextInputType.number, validator: (v) { if (v == null || v.isEmpty) return 'KM Akhir tidak boleh kosong'; final endKm = int.tryParse(v); if (endKm == null) return 'KM Akhir harus berupa angka'; if (widget.trip.startKm != null && int.tryParse(v)! <= widget.trip.startKm!) return 'KM Akhir harus > KM Awal (${widget.trip.startKm})'; return null; }),
          const SizedBox(height: 24),
          
          if (!kmAkhirStatus.isApproved)
            _PhotoSection(title: 'Foto KM Akhir', icon: Icons.camera_alt_outlined, onImageChanged: (file) => setState(() => _kmAkhirImage = file), rejectionReason: kmAkhirStatus.rejectionReason, isApproved: kmAkhirStatus.isApproved),
          if (kmAkhirStatus.isApproved)
            _ApprovedDocumentPlaceholder(title: 'Foto KM Akhir'),
          const SizedBox(height: 24),

          if (!bongkarStatus.isApproved)
            _MultiPhotoSection(title: 'Foto Bongkar Barang (Bisa lebih dari 1)', icon: Icons.inventory_outlined, onImagesChanged: (files) => setState(() => _bongkarBarangImages = files), rejectionReason: bongkarStatus.rejectionReason, isApproved: bongkarStatus.isApproved),
          if (bongkarStatus.isApproved)
            _ApprovedDocumentPlaceholder(title: 'Foto Bongkar Barang'),
          const SizedBox(height: 24),

          if (!suratJalanAkhirStatus.isApproved)
            _MultiPhotoSection(title: 'Foto Surat Jalan Akhir (Bisa lebih dari 1)', icon: Icons.document_scanner_outlined, onImagesChanged: (files) => setState(() => _suratJalanAkhirImages = files), rejectionReason: suratJalanAkhirStatus.rejectionReason, isApproved: suratJalanAkhirStatus.isApproved),
          if (suratJalanAkhirStatus.isApproved)
...
class _PhotoSection extends StatefulWidget {
  final String title; final String? rejectionReason; final IconData icon; final ValueChanged<File?> onImageChanged; final bool isApproved;
  const _PhotoSection({required this.title, required this.icon, required this.onImageChanged, this.rejectionReason, this.isApproved = false});
  @override
...
class _PhotoSectionState extends State<_PhotoSection> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      if (widget.isApproved) const Padding(padding: const EdgeInsets.only(top: 8.0), child: Text('Disetujui', style: TextStyle(color: Colors.green, fontWeight: FontWeight.bold), textAlign: TextAlign.center)),
...
class _MultiPhotoSection extends StatefulWidget {
  final String title; final String? rejectionReason; final IconData icon; final ValueChanged<List<File>> onImagesChanged; final bool isApproved;
  const _MultiPhotoSection({required this.title, required this.icon, required this.onImagesChanged, this.rejectionReason, this.isApproved = false});
  @override
...
class _MultiPhotoSectionState extends State<_MultiPhotoSection> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (widget.isApproved) const Padding(padding: const EdgeInsets.only(top: 8.0), child: Text('Disetujui', style: TextStyle(color: Colors.green, fontWeight: FontWeight.bold))),
      const SizedBox(height: 12),
